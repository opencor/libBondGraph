...
std::string getMathML(const RCPLIB::RCP<const SymEngine::Basic> &expr)
{
    ...
    // boost::regex lx(R"(<apply><times\/><cn cellml:units=\"dimensionless\">-1<\/cn><ci>([\w]+)<\/ci><\/apply>)");
...
std::vector<std::string> getMathML(std::map<RCPLIB::RCP<const SymEngine::Basic>, RCPLIB::RCP<const SymEngine::Basic>, SymEngine::Comparator> &expressions)
{
    std::vector<std::string> mexpr;
    std::ostringstream ss;
    for (auto expr : expressions) {
 ...
 * import statement, connection statement, cellml statement (if import statement, this will be ""), variable name, timevariable name to be mapped (if none, this will be "") bool (true if dot_vname is to be mapped)
 * @return std::tuple<std::string, std::string, std::string, std::string,std::string,bool>
 */
std::tuple<std::string, std::string, std::string, std::string, std::string, bool> generateLinkages(std::string vname, std::tuple<std::string, std::string, char> &prop)
{
    std::string value = std::get<1>(prop);
    //std::cerr << vname <<"\t"<<std::get<0>(prop)<<"\t"<<std::get<1>(prop)<<"\t"<<std::get<2>(prop)<<std::endl;
    try {
        ...
        if (std::get<2>(prop) == 'c') { // Control variable
            ...
            if(j.contains("stateValue") && j["stateValue"]){ //State variable mapping
                //in this case map to a variable named vname_smap, which will be provided as init for the state variable
                connection = "<map_variables variable_1 = \"" + targetVar + "\" variable_2 = \"" + vname + "_smap\" />";
            }
            ...
            return std::make_tuple(filename + ":" + import, compartment + ":" + referenceName + ":" + connection, "", vname, timeVariable, derivativeMapped);
        } else {
            std::string compartment = j["compartment"];
...
getCellML(std::string modelName, const RCPLIB::RCP<BondGraphInterface> &host_, ComputeEquationResults &bgequations)
{
    const RCPLIB::RCP<BondGraph> host = RCPLIB::rcp_dynamic_cast<BondGraph>(host_);
    bool solvable = bgequations.bondGraphValidity;
    std::map<RCPLIB::RCP<const SymEngine::Basic>, RCPLIB::RCP<const SymEngine::Basic>, SymEngine::Comparator> &equations = bgequations.dof;
    std::map<RCPLIB::RCP<const SymEngine::Basic>, RCPLIB::RCP<const SymEngine::Basic>, SymEngine::Comparator> &dofConstrains = bgequations.dof_constraints;
    std::map<RCPLIB::RCP<const SymEngine::Basic>, RCPLIB::RCP<const SymEngine::Basic>, SymEngine::Comparator> &bondEquations = bgequations.bondEquations;
    SymEngine::vec_basic &constraints = bgequations.constraints;
    std::unordered_map<std::string, std::tuple<std::string, std::string, char>> dimensions = bgequations.physicalDimensions;
    ...
    mapParameterFile << "<model name=\"" << modelName << "Parameters \" xmlns=\"http://www.cellml.org/cellml/1.1#\" xmlns:cellml=\"http://www.cellml.org/cellml/1.1#\" xmlns:xlink=\"http://www.w3.org/1999/xlink\">" << std::endl;
    CellMLUnits unitMap;
    ...
    std::map<std::string, std::tuple<std::string, std::string, units::precise_unit>> cellMLDefinitions;
    ...
    for (auto var : orderedDim) {
        ...
        try {
            if (dim != "") {
                ...
                definedNames[var.first] = unitName;
                // mapUnits[unitName] = true; //dimensionless is predefined, so no need to define it
    ...
    connect << "<import xlink:href = \""
            << modelName << "Parameters.cellml\" >" << std::endl;
    ...
    connect << "<connection>\n<map_components component_1 = \"parameters\" component_2 = \"" << modelName << "_main\" />" << std::endl;
    ...
    for (auto var : orderedDim) {
        ...
        if (value.find('{') != std::string::npos) {
            ...
            // Cut to basename
            //  std::string baseFilename = importFile.substr(importFile.find_last_of("/\\") + 1);
            ...
            if (std::get<2>(var.second) == 'c' && std::get<5>(g)) { // Create a map only if a mapping is available
    ...
    for (auto c : timeVariableMap) {
        std::string connection = "<connection>\n<map_components component_1 = \"" + c.first + " \" component_2 =\"time\" />";
        connection = connection + "<map_variables variable_1=\"" + c.second + "\" variable_2=\"t\"/>";
        connection = connection + "</connection>";
    ...
    for (auto c : connectionMap) {
        ...
        for (auto v : vmap) {
            std::string connection = "<connection>\n<map_components component_1 = \"" + v.first + "_IMP\" component_2 =\"" + modelName + "_main\" />";
            for (auto x : v.second) {
    ...
    cellML << "<model name=\"" << modelName << "\" xmlns=\"http://www.cellml.org/cellml/1.1#\" xmlns:cellml=\"http://www.cellml.org/cellml/1.1#\" xmlns:xlink=\"http://www.w3.org/1999/xlink\">" << std::endl;
    // Define units
    ...
    encapsulationDef << "<group><relationship_ref relationship=\"encapsulation\"/><component_ref component=\"" << modelName << "_main\">";
    ...
    // Time connection
    cellML << "<connection><map_components component_1=\"time\" component_2=\"" << modelName << "_main\"/><map_variables variable_1=\"t\" variable_2=\"t\"/></connection>";
    // Define other blocks
    for (auto c : blocks) {
        cellML << c << std::endl;
    }
    ...
    cellML << "<component name=\"time\">  <variable name=\"t\" public_interface=\"out\" units=\"second\"/> </component>";
    ...
    cellML << "<variable name=\"t\" units=\"second\" public_interface = \"in\"/>" << std::endl;
        ...
        case 's': {
            if (var.first.size() > 4 && var.first.substr(0, 4) == "dot_") { // Derivatives are computed
                ...
                if (value.find(":") == std::string::npos) // If value is not json
                    ...
                    cellML << "< variable  name=\"" << var.first << "_smap\" units=\"" << unitName << "\" public_interface = \"in\" />";
                    cellML << "< variable  name=\"" << var.first << "\" units=\"" << unitName << "\" public_interface = \"out\"" << "initial_value = \"" + var.first + "_smap\" />";
                }
            }
            break;
        }
    ...
    if (dofConstrains.size() > 0) {
        cellML << "<!-- List of state variables that have an algebriac solution, select the ode or algebraic version as desired -->" << std::endl;
        ...
        //     else
        //         cellML << "<!--"
        //                << "<apply><eq/><apply><diff/><bvar><ci>t</ci></bvar><ci>" << expr.first << "</ci></apply><ci>" << expr.second << "</ci></apply> "
        //                << "-->" << std::endl;
        // }
        for (auto expr : derivatives) {
            cellML << "<apply><eq/><apply><diff/><bvar><ci>t</ci></bvar><ci>" << expr.first << "</ci></apply><ci>" << expr.second << "</ci></apply> " << std::endl;
