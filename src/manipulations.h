                                                                                                                                                      ...
                                                                                                                                                      SymEngine::map_basic_basic &usedNames)
{
    ...
    /*
            ...
            Ff the term has a dof entry in the denominator or as a argument to a function or is multiplied 
...
/**
     ...
     */
std::vector<ExpressionTermsMap> getLinearAndNonlinearTerms(const RCPLIB::RCP<BondGraphElementBase> &elem, std::unordered_map<std::string, long int> &coordinates)
{
    ...
    */
    ...
    //std::cerr<<__LINE__<<" Used names"<<std::endl;
    for (auto &un : usedNames) {
        //std::cerr<<un.first<<"\t"<<un.second<<std::endl;
...
SmithNormalForm getSmithNormalForm(const SymEngine::DenseMatrix &linear, const SymEngine::DenseMatrix &nonlinear)
{
    ...
    for (int i = 0; i < nonlinearop.nrows(); i++)
        for (int j = 0; j < nonlinearop.ncols(); j++)
            nonlinearop.set(i, j, SymEngine::expand(nonlinearop.get(i, j)));
...
inline SymEngine::RCP<const SymEngine::Basic> substituteValues(SymEngine::RCP<const SymEngine::Basic> &expr, SymEngine::map_basic_basic &substitutions)
{
    ...
    auto newString = replaceAll(replaceAll(expString, "exp(log", "("), "log(exp", "");
    auto nExpr = SymEngine::expand(SymEngine::parse(newString));
    return nExpr;
}
                                    ...
                                    unsigned int numControlVars)
{
    ...
    coordinates.submatrix(coord_atoms, 0, 0, offset + ss_size - 1, 0); //auto coord_atoms = set(coordinates[0:offset + ss_size])
                          ...
                          std::unordered_map<std::string, std::tuple<std::string, std::string, char>> &dimensions, char vtype = 'u')
{
    if (!SymEngine::is_a<SymEngine::Add>(*SymEngine::expand(eq))) {
        std::ostringstream ss;
        //Handle exp and log, symengine subs will zero the equation if the argument to log is unit or make exp into E
        ss << *eq;
        std::string equ = ss.str();
        auto equation = SymEngine::parse(replaceAll(replaceAll(equ, "log", ""), "exp", ""));
        auto atoms = SymEngine::atoms<SymEngine::FunctionSymbol, SymEngine::Symbol>(*equation);
        //auto atoms = SymEngine::free_symbols(*eq);
        ...
        for (auto c : atoms) {
            ...
            if (dim != "") { //Handle dimensionless entities
                try {
                    ...
                    std::cerr << "Failed to find dimensions for " << ss.str() << "\t";
        ...
        //std::cout<<" Measure "<<expString<<"\t"<< *eq << "\t" << expString << " = " << units::to_string(units::precise_unit(baseU,mult))<<"\t"<< units::to_string(units::precise_unit(baseU))<<"\t"<<mult << std::endl;
