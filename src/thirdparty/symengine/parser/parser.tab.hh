...
switch (yykind)
    ...
    parser (SymEngine::Parser &p_yyarg);
    virtual ~parser ();
    ...
    // Implementation of make_symbol for each token kind.
      ...
      static
      symbol_type
      make_IMPLICIT_MUL (std::string v)
      {
        return symbol_type (token::IMPLICIT_MUL, std::move (v));
      }
#else
      static
      symbol_type
      make_IMPLICIT_MUL (const std::string& v)
      {
        return symbol_type (token::IMPLICIT_MUL, v);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
