...
namespace SymEngine{
    //Round about to not expose symengine and its dependencies (boost etc) for downstream applications
    bool Comparator::operator()(const RCPLIB::RCP<const Basic> &x, const RCPLIB::RCP<const Basic> &y) const{
...
RCPLIB::RCP<BondGraphInterface> BondGraph::clone()
{
    ...
    bgGraph->cloned = true;
    std::unordered_map<std::string, std::tuple<RCPLIB::RCP<BGElement> , RCPLIB::RCP<BGElement> >> componentIdMAP;
    for (auto &c : mComponents) {
    ...
    for (auto &b : mBonds) {
        ...
        auto newBond = bgGraph->connect(std::get<1>(fromComp), std::get<1>(toComp), opin, ipin);
...
void BondGraph::addBondGraph(const RCPLIB::RCP<BondGraphInterface> &inComponent_)
{
    const RCPLIB::RCP<BondGraph> inComponent = RCPLIB::rcp_dynamic_cast<BondGraph>(inComponent_);
...
void BondGraph::removeBondGraph(const RCPLIB::RCP<BondGraphInterface> &bg_)
{
    ...
    std::map<std::string, std::vector<RCPLIB::RCP<BGElement> >::iterator> rComponents; //!< List of all elements
    for (std::vector<RCPLIB::RCP<BGElement> >::iterator c = mComponents.begin(); c != mComponents.end(); c++) {
    ...
    for (std::vector<RCPLIB::RCP<BondInterface>>::iterator c = bg->mBonds.begin(); c != bg->mBonds.end(); c++) {
...
/*! \brief Connect two components.
	...
	*/
RCPLIB::RCP<BondInterface> BondGraph::connect(const RCPLIB::RCP<BGElement>  &inOrigin_, const RCPLIB::RCP<BGElement>  &inDest_, int opin_, int ipin_)
{
    RCPLIB::RCP<BondGraphElementBase>  inOrigin = RCPLIB::rcp_dynamic_cast<BondGraphElementBase>(inOrigin_);
    RCPLIB::RCP<BondGraphElementBase>  inDest = RCPLIB::rcp_dynamic_cast<BondGraphElementBase>(inDest_);
    int opin = opin_;
    int ipin = ipin_;
    //Create both ports
    ...
    if (inOrigin->getParent() != "" && inOrigin->getComponentGroup() != eJ && inOrigin->getParent() != id) {
        ...
        if (oPorts.size() == 1) { //Single port element, find the connecting element from the bond
            ...
            inOrigin = RCPLIB::rcp_dynamic_cast<BondGraphElementBase>(oend->getComponent());
    ...
    if (inDest->getParent() != "" && inDest->getComponentGroup() != eJ && inDest->getParent() != id) {
        ...
        if (oPorts.size() == 1) { //Single port element, find the connecting element from the bond
            ...
            const RCPLIB::RCP<PortInterface> &oend = bond->getOtherEnd(rp);
            inDest = RCPLIB::rcp_dynamic_cast<BondGraphElementBase>(oend->getComponent());
    ...
    if (inDest->getComponentGroup() != eJ) {
        ...
        lToPort = createPort(true);
        inDest->connect(lToPort,ipin); //Add to element port list, which then links the element with the port
    }
...
void BondGraph::removeComponent(std::vector<RCPLIB::RCP<BGElement> > &inComponents)
{
...
{
    std::vector<RCPLIB::RCP<BGElement> >::iterator lComponentIter = find(mComponents.begin(), mComponents.end(), inComponent);
...
void BondGraph::reconnectComponent(const RCPLIB::RCP<PortInterface> &inComponentPort, const RCPLIB::RCP<BGElement>  &inNewOrigin)
{
    ...
    const RCPLIB::RCP<PortInterface> &lMovingPort = lBond->getOtherEnd(inComponentPort);
...
std::string BondGraph::serializeAsCellML(std::string modelName)
{
    ...
    std::map<std::string,std::string> res =  getCellML(modelName, interface, result);
    return "";
}
...
RCPLIB::RCP<BondInterface> BondGraph::connect(const RCPLIB::RCP<BGElement>  &inOrigin, int opin, const RCPLIB::RCP<BGElement>  &inDest, int ipin)
{
    return connect(inOrigin, inDest, opin, ipin);
};
RCPLIB::RCP<BondInterface> BondGraph::connect(const RCPLIB::RCP<BGElement>  &inOrigin, int opin, const RCPLIB::RCP<BGElement>  &inDest)
{
    return connect(inOrigin, inDest, opin, 0);
};
RCPLIB::RCP<BondInterface> BondGraph::connect(const RCPLIB::RCP<BGElement>  &inOrigin, const RCPLIB::RCP<BGElement>  &inDest, int ipin)
{
    return connect(inOrigin, inDest, 0, ipin);
};
RCPLIB::RCP<BondInterface> BondGraph::connectInverting(const RCPLIB::RCP<BGElement>  &inOrigin, int opin, const RCPLIB::RCP<BGElement>  &inDest)
{
    return connect(inOrigin, inDest, opin, 0);
};
RCPLIB::RCP<BondInterface> BondGraph::connectInverting(const RCPLIB::RCP<BGElement>  &inOrigin, const RCPLIB::RCP<BGElement>  &inDest, int ipin)
{
    return connect(inOrigin, inDest, 0, ipin);
};

std::vector<std::tuple<std::string, const RCPLIB::RCP<BGElement>  &, bool>> BondGraph::getComponents()
{
    std::vector<std::tuple<std::string, const RCPLIB::RCP<BGElement>  &, bool>> result;
    for (auto c : mComponents) {
...
RCPLIB::RCP<BGElement> __createResistance_do_not_call_outside_of_lib(const RCPLIB::RCP<BGElementData> &proxy)
{
...
RCPLIB::RCP<BGElement> __createCapacitance_do_not_call_outside_of_lib(const RCPLIB::RCP<BGElementData> &proxy)
{
    static unsigned int ctr = 0;
...
RCPLIB::RCP<BGElement> __createInductance_do_not_call_outside_of_lib(const RCPLIB::RCP<BGElementData> &proxy)
{
    static unsigned int ctr = 0;
...
RCPLIB::RCP<BGElement> createPotentialSource(const RCPLIB::RCP<BGElementData> &proxy)
{
    static unsigned int ctr = 0;
...
RCPLIB::RCP<BGElement> createFlowSource(const RCPLIB::RCP<BGElementData> &proxy)
{
    static unsigned int ctr = 0;
...
RCPLIB::RCP<BGElement> createConcentration(const RCPLIB::RCP<BGElementData> &proxy)
{
    static unsigned int ctr = 0;
...
RCPLIB::RCP<BGElement> createTransformer(const RCPLIB::RCP<BGElementData> &proxy)
{
    static unsigned int ctr = 0;
...
RCPLIB::RCP<BGElement> createChemostat(const RCPLIB::RCP<BGElementData> &proxy)
{
    static unsigned int ctr = 0;
...
RCPLIB::RCP<BGElement> createUserDefined(const RCPLIB::RCP<BGElementData> &proxy)
{
    static unsigned int ctr = 0;
...
RCPLIB::RCP<BGElement> createPortHamiltonian(std::string expr,
                                                   std::vector<std::string> states, const RCPLIB::RCP<BGElementData> &proxy)
{
    static unsigned int ctr = 0;
...
RCPLIB::RCP<BGElement> createOneJunction(const RCPLIB::RCP<BGElementData> &proxy)
{
    static unsigned int ctr = 0;
...
RCPLIB::RCP<BGElement> createZeroJunction(const RCPLIB::RCP<BGElementData> &proxy)
{
    static unsigned int ctr = 0;
...
RCPLIB::RCP<BGElement>  createProxy(const RCPLIB::RCP<BGElement>  &obj_)
{
    ...
    const RCPLIB::RCP<BondGraphElementBase>  obj = RCPLIB::rcp_dynamic_cast<BondGraphElementBase>(obj_);
...
RCPLIB::RCP<BGElement>  createBondgraphElement(std::string methodName,std::string expr)
{
...
std::string checkUnits(std::string unit, std::string target){
    ...
    if(compare.base_units()!=targetUnit.base_units()){ //Compare the base units as multiplies can be different
