...
Resistance::Resistance(const RCPLIB::RCP<BGElementData>& data, std::string inId, bool proxy_)
...
void Resistance::connect(const RCPLIB::RCP<PortInterface> &inPort, int portNum)
{
    if (mPorts.size() > 0)
...
Capacitance::Capacitance(const RCPLIB::RCP<BGElementData>& data, std::string inId, bool proxy_)
    : BondGraphElementBase(data, inId)
{
    proxy = proxy_;
    if (proxy)
...
void Capacitance::connect(const RCPLIB::RCP<PortInterface> &inPort, int portNum)
{
    if (mPorts.size() > 0)
...
Inductance::Inductance(const RCPLIB::RCP<BGElementData>& data, std::string inId, bool proxy_)
    : BondGraphElementBase(data, inId)
{
    proxy = proxy_;
    if (proxy)
...
void Inductance::connect(const RCPLIB::RCP<PortInterface> &inPort, int portNum)
{
    if (mPorts.size() > 0)
...
PotentialSource::PotentialSource(const RCPLIB::RCP<BGElementData>& data, std::string inId, bool proxy_)
    : BondGraphElementBase(data, inId)
{
    proxy = proxy_;
    if (proxy)
...
void PotentialSource::connect(const RCPLIB::RCP<PortInterface> &inPort, int portNum)
{
    if (mPorts.size() > 0)
...
FlowSource::FlowSource(const RCPLIB::RCP<BGElementData>& data, std::string inId, bool proxy_)
    : BondGraphElementBase(data, inId)
{
    proxy = proxy_;
    if (proxy)
...
void FlowSource::connect(const RCPLIB::RCP<PortInterface> &inPort, int portNum)
{
    if (mPorts.size() > 0)
...
Concentration::Concentration(const RCPLIB::RCP<BGElementData>& data, std::string inId, bool proxy_)
    : BondGraphElementBase(data, inId)
{
    proxy = proxy_;
    if (proxy)
    ...
    //Biochemical Const
    {
        ...
        auto mult = un.multiplier();
        auto baseU = un.base_units();
        mBiochemicalConst->units = units::to_string(units::precise_unit(baseU, mult));
    }
    setName("Ce");
...
void Concentration::connect(const RCPLIB::RCP<PortInterface> &inPort, int portNum)
{
    if (mPorts.size() > 0)
...
ChemicalSource::ChemicalSource(const RCPLIB::RCP<BGElementData>& data, std::string inId, bool proxy_)
    : BondGraphElementBase(data, inId)
{
    proxy = proxy_;
    if (proxy)
...
void ChemicalSource::connect(const RCPLIB::RCP<PortInterface> &inPort, int portNum)
{
    if (mPorts.size() > 0)
        throw BGException("Attempt to connect Single port component (Chemostat) to more than one port!");
    mPorts.push_back(inPort);
    inPort->connect(rcpPtr);
}

void ChemicalSource::setDof(size_t inId) {
    SETIDANDUPDATE(inId, mParameter)
}


ChemicalFlux::ChemicalFlux(const RCPLIB::RCP<BGElementData>& data, std::string inId, bool proxy_)
    : BondGraphElementBase(data, inId)
{
    proxy = proxy_;
    if (proxy)
...
void ChemicalFlux::connect(const RCPLIB::RCP<PortInterface> &inPort, int portNum)
{
    if (mPorts.size() > 0)
        throw BGException("Attempt to connect Single port component (Flowstat) to more than one port!");
    mPorts.push_back(inPort);
    inPort->connect(rcpPtr);
}

void ChemicalFlux::setDof(size_t inId) {
    SETIDANDUPDATE(inId, mParameter)
}
...
Stoichiometry::Stoichiometry(const RCPLIB::RCP<BGElementData>& data, std::string inId, bool proxy_)
    : BondGraphElementBase(data, inId)
{
    proxy = proxy_;
    if (proxy)
    ...
    mParameter.push_back(parameter);
    parameter->units = units::to_string(units::precise::one); //same as dimensionless
    RCPLIB::RCP<Value> parameter1 = RCPLIB::rcp(new Value("r1", -1.0));
    mParameter.push_back(parameter1);
    parameter1->units = units::to_string(units::precise::one); //same as dimensionless
...
void Stoichiometry::connect(const RCPLIB::RCP<PortInterface> &inPort, int portNum)
{
    if (portNum > 1)
        throw BGException("Attempt to connect Two port component (Stoichiometry)  to more than two ports!");
    mPorts.push_back(inPort);
    inPort->connect(rcpPtr);
}

void Stoichiometry::setDof(size_t inId) {
    SETIDANDUPDATE(inId, mParameter)
}


Transformer::Transformer(const RCPLIB::RCP<BGElementData>& data, std::string inId, bool proxy_)
    : BondGraphElementBase(data, inId)
{
    proxy = proxy_;
    if (proxy)
    ...
    constitutiveEqIndex = {-1, -1};
    //Should match with the element metadata definition by generateFactoryMethods.py
    auto p1 = createPort(true);
    p1->setId("in");
    mPorts.push_back(p1);
    auto p2 = createPort(false);
...
void Transformer::connect(const RCPLIB::RCP<PortInterface> &inPort, int portNum)
{
    if (portNum > 1)
...
Gyrator::Gyrator(const RCPLIB::RCP<BGElementData>& data, std::string inId, bool proxy_)
    : BondGraphElementBase(data, inId)
{
    proxy = proxy_;
    if (proxy)
...
ZeroJunction::ZeroJunction(const RCPLIB::RCP<BGElementData>& data, std::string inId, bool proxy_)
    : BondGraphElementBase(data, inId)
{
    proxy = proxy_;
    if (proxy)
...
void ZeroJunction::connect(const RCPLIB::RCP<PortInterface> &inPort, int portNum)
{
    portModified = true;
    mPorts.push_back(inPort);
    inPort->connect(rcpPtr);
...
*/
OneJunction::OneJunction(const RCPLIB::RCP<BGElementData>& data, std::string inId, bool proxy_)
    : BondGraphElementBase(data, inId)
{
    proxy = proxy_;
    if (proxy)
...
void OneJunction::connect(const RCPLIB::RCP<PortInterface> &inPort, int portNum)
{
    portModified = true;
    mPorts.push_back(inPort);
    inPort->connect(rcpPtr);
...
Reaction::Reaction(const RCPLIB::RCP<BGElementData>& data, std::string inId, bool proxy_)
    : BondGraphElementBase(data, inId)
{
    proxy = proxy_;
    if (proxy)
...
UserDefined::UserDefined(const RCPLIB::RCP<BGElementData>& data, std::string inId, bool proxy_)
    : BondGraphElementBase(data, inId)
{
    proxy = proxy_;
    if (proxy)
...
void UserDefined::connect(const RCPLIB::RCP<PortInterface> &inPort, int portNum)
{
...
void UserDefined::setPhysicalDomain(PhysicalDomain domain)
{
    if (portDimensions.find(domain) != portDimensions.end()) {
        ...
        mParameter[0]->units = units::to_string(units::unit_from_string(std::get<0>(res)));
        mParameter[1]->units = units::to_string(units::unit_from_string(std::get<1>(res)));
    } else {
...
PortHamiltonian::PortHamiltonian(std::string hexpr,
                                 ...
                                 const RCPLIB::RCP<BGElementData>& data, std::string inId, bool proxy_)
    : BondGraphElementBase(data, inId)
{
    proxy = proxy_;
    if (proxy)
    ...
    for (int i = 0; i < states.size(); i++) {
        ...
        mValue->units = units::to_string(units::precise::one); //Same as dimensionless
        mParameter.push_back(mValue);
    }
...
void PortHamiltonian::connect(const RCPLIB::RCP<PortInterface> &inPort, int portNum)
{
    if (mPorts.size() > maxPorts) {
        std::ostringstream ss;
        ss << "Userdefined " << maxPorts << "'s component connected to more than " << maxPorts << "!" << std::endl;
