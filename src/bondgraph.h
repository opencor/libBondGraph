...
/*! \brief %Bond Graph
                ...
                To define a bond graph, individual elements need to be
   instantiated and added to the collection The elements should then be
   connected together using the connect method.
        ...
        ioBondGraph->connectInverting(Re, 1, Y_B);
                ...
                The bond graph is represented as a linked list. All elements of
   the bond graph are interconnected through ports. Each port connects to a
   Component via a Bond. Therefore, two component are linked like this :
...
private:
  ...
  bool cloned = false; //! True when bondgraph is cloned, getComponents will
                       //! return proxies
  std::string ownerID =
      ""; //! ID of the bondgraph instance, if this instance is a component in
          //! another bondgraph with id=ownerID, sanity checking to avoid
          //! sharing one instance with multiple parents
protected:
  unsigned int mElementIdCounter = 1; //!< Id counter for elements
  unsigned int mBondIdCounter = 1;    //!< Id counter for bonds
  RCPLIB::RCP<NGraph::sGraph>
      graph; // Graph data structure to capture connectivity
...
public:
  ...
  connect(const RCPLIB::RCP<BGElement> &inOrigin,
          const RCPLIB::RCP<BGElement> &inDest, int opin = 0, int ipin = 0);
  /**
   * @brief Connect the power ports of two bondgraph elements that exist within
   * the bondgraph
   *
   ...
   */
  virtual void
  removeComponent(std::vector<RCPLIB::RCP<BGElement>> &inComponents);
  /**
   ...
   * Use the algorithm published in
   * M. Pfeifer, S. Caspert, S. Pfeiffer, C. Muller, S. Krebs, S. Hohmann:
   * "Automated Generation of ExplicitPort-Hamiltonian Models from Multi-Bond
   ...
   * Returns a tuple with either the readableName (if available) or id, handle
   * to it and whether it is a proxy
  ...
  friend void to_json(nlohmann::json &j,
                      const RCPLIB::RCP<BondGraphInterface> &p);
  friend void from_json(const nlohmann::json &j,
                        RCPLIB::RCP<BondGraphInterface> &p);
  friend RCPLIB::RCP<BondGraphInterface> createBondGraph();
};

/**
 * @brief Create a Resistane instance
 * Library users are not expected to call this as element definition is
 * incomplete and domain is not assigned. Users should call one of the generated
 * factory methods
