...
ComputeEquationResults BondGraph::computeStateEquation()
{
    ...
    std::unordered_map<std::string, RCPLIB::RCP<const SymEngine::Basic>> nonlinearTermSubs;
    ...
    // Assign unique dof values to the components to be pulled into a system of equations
    for (auto &mc_ : connectedComponents) {
        const RCPLIB::RCP<BondGraphElementBase> mc = RCPLIB::rcp_dynamic_cast<BondGraphElementBase>(mc_);
    ...
    for (auto &mc_ : connectedComponents) {
        const RCPLIB::RCP<BondGraphElementBase> mc = RCPLIB::rcp_dynamic_cast<BondGraphElementBase>(mc_);
#ifdef  DEBUG_COMPUTESTATEEQUATION
        std::vector<ExpressionTermsMap> cxt = getLinearAndNonlinearTerms(mc, coordinateMap);
        cIndexes[rows] = cxt;
        ...
        for(auto& lc: lceq){
            ...
            cei++;
            {
                for(int ic=0;ic<cc.indexes.size();ic++){
                    std::cout<<"\t"<<rccc<<"\t"<<cc.indexes[ic]<<"\t"<<*cc.coefficients[ic]<<std::endl;
                }
            }
            rccc++;
        }        
        ...
        for (auto &mx : mp.second) {
            for (int i = 0; i < mx.indexes.size(); i++) {
                ...
                if (col > -1) {
                    ...
                    nonlinearTerms.set(rowIx, 0, SymEngine::add(nonlinearTerms.get(rowIx, 0), cef));
                }
            }
            rowIx++;
        }
    ...
    for (auto &mc_ : connectedComponents) {
        const RCPLIB::RCP<BondGraphElementBase> mc = RCPLIB::rcp_dynamic_cast<BondGraphElementBase>(mc_);
        if (mc->getComponentGroup() == ePH) {
            RCPLIB::RCP<PortHamiltonian> ph = RCPLIB::rcp_dynamic_cast<PortHamiltonian>(mc_);
    ...
    //The following are solutions for the state variables - can be ignored if dot_dof is defined
    ...
    *computedCoordinates = coordinates;
    ...
    for (auto &k : equations) {
        ...
        //results.dof[k.first] = SymEngine::simplifyExpLog(k.second->subs(beqSubs));
        //results.dof[k.first] = SymEngine::simplifyExpLog(k.second);
        //results.dof[k.first] = k.second;
    }
    ...
    for (auto &k : bondEquations) {
        ...
        results.bondEquations[k.first->subs(nameMapSubs)] = SymEngine::simplifyExpLog(k.second)->subs(nameMapSubs);
    }
    //Dof solutions
    for (auto &k : eqConstraints) {
        results.dof_constraints[k.first->subs(nameMapSubs)] = SymEngine::simplifyExpLog(k.second)->subs(nameMapSubs);
    }
