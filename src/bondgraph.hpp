...
namespace SymEngine{
    ...
    class Basic;
    struct Comparator{
        bool operator()(const RCPLIB::RCP<const Basic> &x, const RCPLIB::RCP<const Basic> &y) const;
    };
}
...
class EXPORTED BondInterface{
    ...
    //! Get the other end of the bond.
     virtual RCPLIB::RCP<PortInterface> getOtherEnd(const RCPLIB::RCP<PortInterface> inPort) const = 0;
    ...
    //! Connect two ports together.
     virtual void connect(const RCPLIB::RCP<PortInterface> &inFromPort, const RCPLIB::RCP<PortInterface> &inToPort) = 0;    
};
...
class EXPORTED BGElement{
    ...
    //! Connect the component to inPort and disconnect it from outPort
     virtual void connect(const RCPLIB::RCP<PortInterface> &inPort, const RCPLIB::RCP<PortInterface> &outPort) = 0;
    ...
    //! Set parameter
     virtual RCPLIB::RCP<Value> setParameter(std::string name, std::string value, std::string unit) = 0;
    //! Set value
    ...
    //! Set a symbol as a Universal Constant
     virtual RCPLIB::RCP<Value> setUniversalConstant(std::string name, double &value, std::string unit) = 0;
...
class EXPORTED ComputeEquationResults{
    public:
    //false if the logic concludes that the bg is incorrect, for instance, some state and power variables are evaluating to zero, it will still return the equations
    bool bondGraphValidity; 
    //Name map
    std::unordered_map<std::string,std::string> nameMap;
    //system of equations corresponding to each dof - dof_symbol, eqn
    std::map<RCPLIB::RCP<const SymEngine::Basic>, RCPLIB::RCP<const SymEngine::Basic>, SymEngine::Comparator> dof;
    //system of equations corresponding to each bond - bond_variable, eqn
    std::map<RCPLIB::RCP<const SymEngine::Basic>, RCPLIB::RCP<const SymEngine::Basic>, SymEngine::Comparator> bondEquations;
    //constraints on equations corresponding to each dof - dof_symbol, eqn
    std::map<RCPLIB::RCP<const SymEngine::Basic>, RCPLIB::RCP<const SymEngine::Basic>, SymEngine::Comparator> dof_constraints;    
    //list of constraints
    std::vector<RCPLIB::RCP<const SymEngine::Basic>> constraints;
    //physical dimensions, value and variable type associated with every the model: the last element identifies if it is a state (s), control (c) or parameter (p)
    std::unordered_map<std::string, std::tuple<std::string, std::string, char>> physicalDimensions;    
};
...
class EXPORTED BondGraphInterface{
     ...
     * @param inComponent Element to be added
     */
     virtual void addBondGraph(const RCPLIB::RCP<BondGraphInterface> &inComponent) = 0;
    /**
     ...
     */
     virtual RCPLIB::RCP<BondInterface> connect(const RCPLIB::RCP<BGElement>  &inOrigin, const RCPLIB::RCP<BGElement>  &inDest, int opin = 0, int ipin = 0) = 0;
    /**
     * @brief Connect the power ports of two bondgraph elements that exist within the bondgraph
     * 
     * @param inOrigin The element from which power is expected to flow out
     ...
     * @return RCPLIB::RCP<BondInterface> The bond connecting the two elements
     */
     virtual RCPLIB::RCP<BondInterface> connect(const RCPLIB::RCP<BGElement>  &inOrigin, int opin, const RCPLIB::RCP<BGElement>  &inDest, int ipin) = 0;
    /**
     * @brief Connect the power ports of two bondgraph elements that exist within the bondgraph
     * 
     * @param inOrigin The element from which power is expected to flow out
     ...
     * @return RCPLIB::RCP<BondInterface> The bond connecting the two elements
     */
     virtual RCPLIB::RCP<BondInterface> connect(const RCPLIB::RCP<BGElement>  &inOrigin, int opin, const RCPLIB::RCP<BGElement>  &inDest) = 0;
    /**
     * @brief Connect the power ports of two bondgraph elements that exist within the bondgraph
     * 
     * @param inOrigin The element from which power is expected to flow out
     ...
     * @return RCPLIB::RCP<BondInterface> The bond connecting the two elements
     */
     virtual RCPLIB::RCP<BondInterface> connect(const RCPLIB::RCP<BGElement>  &inOrigin, const RCPLIB::RCP<BGElement>  &inDest, int ipin) = 0;
    /**
     * @brief Connect the power ports of two bondgraph elements that exist within the bondgraph
     * 
     * @param inOrigin The element from which power is expected to flow out
     ...
     * @return RCPLIB::RCP<BondInterface> The bond connecting the two elements
     */
     virtual RCPLIB::RCP<BondInterface> connectInverting(const RCPLIB::RCP<BGElement>  &inOrigin, int opin, const RCPLIB::RCP<BGElement>  &inDest) = 0;
    /**
     * @brief Connect the power ports of two bondgraph elements that exist within the bondgraph
     * 
     * @param inOrigin The element from which power is expected to flow out
     ...
     * @return RCPLIB::RCP<BondInterface> The bond connecting the two elements
     */
     virtual RCPLIB::RCP<BondInterface> connectInverting(const RCPLIB::RCP<BGElement>  &inOrigin, const RCPLIB::RCP<BGElement>  &inDest, int ipin) = 0;
     ...
     */
     virtual void removeComponent(std::vector<RCPLIB::RCP<BGElement> > &inComponents) = 0;
    /**
     ...
     */
     virtual void reconnectComponent(const RCPLIB::RCP<PortInterface> &inComponentPort, const RCPLIB::RCP<BGElement>  &inNewOrigin) = 0;
     ...
     * Returns a tuple with readablename (if not available the id), handle to it and whether it is a proxy
     * @return std::vector<std::tuple<str::string,RCPLIB::RCP<BGElement> ,bool>>& 
     */
     virtual std::vector<std::tuple<std::string,const RCPLIB::RCP<BGElement> &,bool>> getComponents() = 0;
...
class EXPORTED ComponentRegistryInterface{
  public:
    virtual void ownComponent(std::string parent, RCPLIB::RCP<BGElement>  comp, std::string readableName = "") = 0;
    virtual void ownBondgraph(std::string parent, RCPLIB::RCP<BondGraphInterface> comp, std::string readableName = "") = 0;
    //These functions do not take ownership of component memory management
    virtual void addComponent(std::string parent, const RCPLIB::RCP<BGElement>  &comp, std::string readableName = "") = 0;
    virtual void addBondgraph(std::string parent, const RCPLIB::RCP<BondGraphInterface> &comp, std::string readableName = "") = 0;
    virtual void setName(const RCPLIB::RCP<BGElement>  &comp, std::string readableName) = 0;
    virtual void setName(const RCPLIB::RCP<BondGraphInterface> &comp, std::string readableName) = 0;
    ...
    virtual std::tuple<int, const RCPLIB::RCP<BGElement>  &, const RCPLIB::RCP<BondGraphInterface> &> getComponentByName(std::string readableName) = 0;
    virtual std::tuple<int, const RCPLIB::RCP<BGElement>  &, const RCPLIB::RCP<BondGraphInterface> &> getComponent(std::string id) = 0;
    virtual RCPLIB::RCP<nlohmann::json> getPropertiesByName(std::string readableName) = 0;
 ...
 */
EXPORTED RCPLIB::RCP<BGElement> createPotentialSource(const RCPLIB::RCP< BGElementData > &proxy=RCPLIB::null);
 ...
 * @return Reference counted pointer to the element 
 */
EXPORTED RCPLIB::RCP<BGElement> createFlowSource(const RCPLIB::RCP< BGElementData > &proxy=RCPLIB::null);
 ...
 * @return Reference counted pointer to the element
 */
EXPORTED RCPLIB::RCP<BGElement> createConcentration(const RCPLIB::RCP< BGElementData > &proxy=RCPLIB::null);
 ...
 * @return Reference counted pointer to the element
 */
EXPORTED RCPLIB::RCP<BGElement> createTransformer(const RCPLIB::RCP< BGElementData > &proxy=RCPLIB::null);
 ...
 * @return Reference counted pointer to the element
 */
EXPORTED RCPLIB::RCP<BGElement> createGyrator(const RCPLIB::RCP< BGElementData > &proxy=RCPLIB::null);
 ...
 * @return Reference counted pointer to the element
 */
EXPORTED RCPLIB::RCP<BGElement> createReaction(const RCPLIB::RCP< BGElementData > &proxy=RCPLIB::null);
 ...
 * @return Reference counted pointer to the element
 */
EXPORTED RCPLIB::RCP<BGElement> createChemostat(const RCPLIB::RCP< BGElementData > &proxy=RCPLIB::null);
 ...
 * @return Reference counted pointer to the element
 */
EXPORTED RCPLIB::RCP<BGElement> createFlowstat(const RCPLIB::RCP< BGElementData > &proxy=RCPLIB::null);
 ...
 * @return Reference counted pointer to the element
 */
EXPORTED RCPLIB::RCP<BGElement> createUserDefined(const RCPLIB::RCP< BGElementData > &proxy=RCPLIB::null);
 ...
 * @return EXPORTED 
 */
EXPORTED RCPLIB::RCP<BGElement> createPortHamiltonian(std::string expr="",
                                                            std::vector<std::string> states={}, const RCPLIB::RCP< BGElementData > &proxy=RCPLIB::null);
 ...
 * @return Reference counted pointer to the element
 */
EXPORTED RCPLIB::RCP<BGElement> createOneJunction(const RCPLIB::RCP< BGElementData > &proxy=RCPLIB::null);
 ...
 * @return Reference counted pointer to the element
 */
EXPORTED RCPLIB::RCP<BGElement> createZeroJunction(const RCPLIB::RCP< BGElementData > &proxy=RCPLIB::null);
 ...
 * @param expr       - porthamiltonian factory method requires an expression, and states other do not. For porthamiltonians's this must be a json object string like {"Hamiltonian":"x^2+y^3-3.0*a","states":["x","y"]}
 * @return EXPORTED 
 */
EXPORTED RCPLIB::RCP<BGElement>  createBondgraphElement(std::string methodName,std::string expr="");
/**
...
/**
 * @brief Check if the unit specification (unit) matches the base unit type (target)
 * @param unit the unit spec that needs to be checked
 * @param target the physical unit against which the check needs to be carried out
 * @return std::string empty if the base units match, else base units of `unit` will be returned
 */
 ...
 * @param host 
 * @param bgequations 
 * @return std::map<std::string, std::string>, a map of filename , contents which can be serialised by the calling logic
 */

EXPORTED std::map<std::string, std::string> getCellML(std::string modelName, const RCPLIB::RCP<BondGraphInterface> &host, ComputeEquationResults &bgequations);
 ...
 * @return std::string 
 */
EXPORTED std::string symEngineExpressionToString(const RCPLIB::RCP<const SymEngine::Basic>& bs);
 ...
 * 
 */
EXPORTED RCPLIB::RCP<BGElement>  loadJson(const nlohmann::json &j, const RCPLIB::RCP<BGElementData> &proxy = RCPLIB::null);
...
EXPORTED void to_json(nlohmann::json &j, const RCPLIB::RCP<BondGraphInterface> &p_);
EXPORTED void from_json(const nlohmann::json &j, RCPLIB::RCP<BondGraphInterface> &p_);
